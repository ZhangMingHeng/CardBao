    ...
    [UIView animateWithDuration:(animated?0.25:0) animations:^{
        switch (self->_indicatorType) {
            ...
            case FSIndicatorTypeNone:
                ...
                break;
            case FSIndicatorTypeEqualTitleFloat:
                self.indicatorView.frame = CGRectMake(selectBtn.center.x-indicatorWidth/2.0, self.scrollView.bounds.size.height - 3.5, indicatorWidth, 2);
                break;
            default:
                break;
        }
...
{
    if (!_indicatorView) {
        _indicatorView = [[UIView alloc]initWithFrame:CGRectMake(0, self.scrollView.bounds.size.height - 1, 0, 1)];
...
- (void)setTitlesArr:(NSArray *)titlesArr
{
    ...
    self.itemBtnArr = nil;
    for (id title in titlesArr) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.tag = self.itemBtnArr.count + 666;
        if ([title isKindOfClass:[NSAttributedString class]]) {
            //富文本
            NSAttributedString *attr = (NSAttributedString *)title;
            NSMutableAttributedString *attrNormal = [[NSMutableAttributedString alloc] initWithAttributedString:title];
            NSMutableAttributedString *attrSelected = [[NSMutableAttributedString alloc] initWithAttributedString:title];
            [attrNormal addAttributes:@{NSForegroundColorAttributeName:_titleNormalColor} range:NSMakeRange(0, attr.length)];
            [attrSelected addAttributes:@{NSForegroundColorAttributeName:_titleSelectColor} range:NSMakeRange(0, attr.length)];
...
{
    if ((_selectIndex == selectIndex && selectIndex != 0)||selectIndex < 0||selectIndex > self.itemBtnArr.count - 1) {
        return;
    }
...
/**
 ...
 @return 字符串长度
 */
+ (CGFloat)getWidthWithString:(id)string font:(UIFont *)font {
    NSDictionary *attrs = @{NSFontAttributeName : font};
    if ([string isKindOfClass:[NSAttributedString class]]) {
        NSAttributedString *attr = (NSAttributedString *)string;
        return [attr.string boundingRectWithSize:CGSizeMake(0, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size.width;
    }else{
        NSString *str = (NSString *)string;
        return [str boundingRectWithSize:CGSizeMake(0, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size.width;
    }
}
