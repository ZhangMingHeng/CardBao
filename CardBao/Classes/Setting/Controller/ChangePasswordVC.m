...
-(void)getUI {
    ...
    // 自定义成导航栏
    [self setNavigationViewTitle:@"修改密码" hiddenBackButton:NO backgroundColor:nil];
...
-(void)submitClick:(UIButton*) sender {
    ...
    //originalPass：愿密码； newPass：账号；
    [[RequestManager shareInstance]postWithURL:CHANGEPASSWORD_INTERFACE parameters:@{@"originalPass":oldPsw.text,@"newPass":newPsw.text} isLoading:YES loadTitle:nil addLoadToView:self.view andWithSuccess:^(RequestManager * _Nonnull manage, id  _Nonnull model) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            // UI线程
            [Helper alertMessage:@"密码修改成功" addToView:self.view];
